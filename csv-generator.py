...
# ------------------ Session State ------------------
for key, default in [('es_client', None), ('data_streams', []), ('indices', []), ('fields', []),
                     ('connected', False), ('extracting', False), ('download_ready', False),
                     ('csv_file', None), ('stats', None)]:
...
st.markdown("Extract data streams **and indices** directly to CSV")
...
with st.sidebar:
    ...
    if auth == "API Key":
        key = st.text_input("Key", value="", type="password")
        user = pwd = None
    ...
    show_sys = st.checkbox("Show system indices (.*)", False,
                           help="Includes hidden and dot-prefixed indices such as .ds-* backing indices")
    ...
    if st.button("🔌 Connect"):
        try:
            ...
            try:
                ...
                st.warning(f"⚠️ Could not list data streams: {de}")
            ...
            try:
                cat = es.cat.indices(
                    ...
                    expand_wildcards="open,hidden" if show_sys else "open"
                )
                for i in cat:
                    name = i.get('index', '')
                    if not name:
                        continue
                    # .ds-* are data stream backing indices, skip unless asked
                    if name.startswith('.') and not show_sys:
                        continue
                    idx_list.append({
                        'name': name,
                        'docs': i.get('docs.count') or '0',
                        'size': i.get('store.size') or '-',
                        'health': i.get('health') or '-',
                    })
                idx_list.sort(key=lambda x: x['name'])
            except Exception as ie:
                st.warning(f"⚠️ Could not list indices: {ie}")

            st.session_state.indices = idx_list
            st.success(f"✅ {len(streams)} data streams, {len(idx_list)} indices found")
        except Exception as e:
    ...
    if st.session_state.connected:
        st.caption(f"📦 {len(st.session_state.data_streams)} streams | 🗄️ {len(st.session_state.indices)} indices")
    st.markdown("---")
...
if not st.session_state.connected:
    ...
    st.stop()
if not st.session_state.data_streams and not st.session_state.indices:
    st.warning("⚠️ No data streams or indices available")
    st.stop()

# ------------------ Source selection (stream / index / pattern) ------------------
st.subheader("📊 Data Source")

src_opts = []
if st.session_state.data_streams:
    src_opts.append("Data Stream")
if st.session_state.indices:
    src_opts.append("Index")
src_opts.append("Custom Pattern")

src = st.radio("Source type", src_opts, horizontal=True, key="source_type")
...
if src == "Data Stream":
    ...
    stream = sel_map[st.selectbox("Stream", opts)]

elif src == "Index":
    idx_filter = st.text_input("Filter indices", placeholder="type to narrow the list, e.g. logs")
    pool = st.session_state.indices
    if idx_filter.strip():
        pool = [i for i in pool if idx_filter.strip().lower() in i['name'].lower()]

    if not pool:
        st.warning("❌ No indices match that filter")
        st.stop()

    opts = [f"{i['name']}  •  {i['docs']} docs  •  {i['size']}  •  {i['health']}" for i in pool]
    sel_map = {o: i['name'] for o, i in zip(opts, pool)}
    stream = sel_map[st.selectbox(f"Index ({len(pool)} shown)", opts)]

    multi = st.checkbox("Select multiple indices instead", value=False)
    if multi:
        picked = st.multiselect("Indices", [i['name'] for i in pool])
        if picked:
            stream = ",".join(picked)
            st.caption(f"🎯 Target: `{stream}`")

else:  # Custom Pattern
    stream = st.text_input(
        "Index / alias / pattern",
        value="logs-*",
        help="Comma-separated names or wildcards, e.g. logs-2025.*,metrics-*"
    ).strip()

    if not stream:
        st.warning("⚠️ Enter an index name or pattern")
        st.stop()

    if st.button("🔎 Resolve pattern"):
        try:
            resolved = st.session_state.es_client.indices.get(index=stream)
            names = list(resolved.keys())
            cnt = st.session_state.es_client.count(index=stream)['count']
            st.success(f"✅ Matches {len(names)} indices, {cnt:,} documents")
            with st.expander("Matched indices"):
                st.code("\n".join(names[:200]), language=None)
        except Exception as e:
            st.error(f"❌ {e}")

st.caption(f"Querying: `{stream}`")
...
with col1:
    ...
    if ttype == "Relative":
        ...
        if tz_option == "Browser Local Time (Auto-detect)":
            ...
            tz_list = ['Asia/Karachi', 'Asia/Riyadh', 'Asia/Dubai', 'US/Eastern', 'US/Central', 'US/Mountain', 'US/Pacific', 'Europe/London', 'Europe/Paris', 'Europe/Berlin', 'Asia/Tokyo', 'Asia/Shanghai', 'Australia/Sydney', 'UTC']
...
success 200"""
    ...
    if kql and kql.strip():
        ...
        if ": *" in kql and ">" in kql:
            warnings_list.append("⚠️ Avoid using `field: * AND field > value`. Just use `field > value` or `field >= value`")
        ...
        for field in matches:
            if f"{field} >" in kql or f"{field} <" in kql or f"{field}:" in kql.replace(f"{field}: *", ""):
                warnings_list.append(f"⚠️ Field `{field}` has both existence check (`: *`) and other conditions. Remove the `: *` part.")
            ...
            if st.button("🎯 Test Query + Time Filter", help="Test exactly what will be extracted", use_container_width=True):
                try:
                    ...
                    if ttype != "None" and tfilter:
                        # Check if we already have a range query
                        if test_query['query'].get('range'):
                            ...
                            test_query['query'] = {
                                'bool': {
                                    'filter': [
                                        {'range': existing_range},  # Keep original field filter
...
with col2:
    ...
    meta = st.checkbox("Include metadata", help="Adds _id, _index and _score columns — useful when querying multiple indices")
    st.subheader("🎯 Fields")
    ...
    if fsel == "Specific" and fields:
        ...
        if st.session_state.get('show_all_fields'):
            with st.expander("📋 All Available Fields (Copy to use in bulk add)", expanded=True):
                ...
                safe_name = str(stream).replace('*', 'star').replace(',', '_').replace('/', '_')
                st.download_button(
                    label="💾 Download Field List",
                    data=all_field_text,
                    file_name=f"{safe_name}_fields.txt",
                    mime="text/plain"
                )
...
if not st.session_state.extracting and (kql or ttype != "None" or (fsel == "Specific" and selected)):
    with st.expander("📋 Extraction Summary", expanded=False):
        ...
        # Source
        st.write(f"🗂️ **Source ({src}):** `{stream}`")

        # Time range
        if ttype == "Relative":
            st.write(f"⏰ **Time Range:** Last {trange}")
        elif ttype == "Absolute":
            st.write(f"⏰ **Time Range:** {tfilter['gte']} → {tfilter['lte']} (UTC)")
        else:
            st.write(f"⏰ **Time Range:** All time")

        # KQL filter
        if kql and kql.strip():
            st.write(f"🔍 **Query Filter:** `{kql.strip()}`")
        else:
            st.write(f"🔍 **Query Filter:** None (all documents)")
...
if btn:
    try:
        ...
        # Add time filter
        if ttype != "None" and tfilter:
            # Check if we already have a simple range query
        ...
        with st.expander("🔍 Query Details (Debug)", expanded=True):
            st.write("**Target:**")
            st.code(str(stream), language=None)
            ...
            if '_source' in query:
                ...
                st.info("ℹ️ _source parameter NOT SET - Will fetch ALL fields")
...
st.sidebar.markdown("---")
st.sidebar.markdown("**v5.0** | Data Streams + Indices + Patterns")
